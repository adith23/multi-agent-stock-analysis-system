...
ANALYSIS_REQUIRED_MACRO_SERIES = tuple(
    env.list(
        "ANALYSIS_REQUIRED_MACRO_SERIES",
        default=["FEDFUNDS", "CPIAUCSL", "GDPC1", "UNRATE", "DGS10", "DGS2", "VIXCLS"],
    )
)
ANALYSIS_DATA_CAPABILITIES = tuple(env.list("ANALYSIS_DATA_CAPABILITIES", default=[]))
ANALYSIS_REQUIRED_OPTIONAL_DATA_CAPABILITIES = tuple(
    env.list("ANALYSIS_REQUIRED_OPTIONAL_DATA_CAPABILITIES", default=[])
)
INGESTION_LOCK_TIMEOUT_SECONDS = env.int("INGESTION_LOCK_TIMEOUT_SECONDS", default=600)
INGESTION_LOCK_WAIT_SECONDS = env.float("INGESTION_LOCK_WAIT_SECONDS", default=30)
