...
function RemoteSpecialistReport({ report }: { report: SpecialistReport }) {
  return <Panel><div className="flex items-center justify-between"><SectionLabel>{report.specialist} specialist output</SectionLabel><span className="flex items-center gap-1 font-mono text-[10px] capitalize text-text-dim" style={{ fontFamily: "var(--font-mono)" }}><StanceIcon stance={report.stance || AgentStance.NEUTRAL} />{report.stance || "unclassified"} · {(report.confidence * 100).toFixed(0)}%</span></div><h3 className="font-mono text-sm font-medium" style={{ fontFamily: "var(--font-mono)" }}>{report.thesis}</h3><p className="mt-3 text-xs leading-6 text-text-dim">{report.summary}</p><div className="mt-4 grid grid-cols-2 gap-5"><div><h4 className="font-mono text-[9px] text-text-faint uppercase" style={{ fontFamily: "var(--font-mono)" }}>Assumptions</h4><pre className="mt-1 whitespace-pre-wrap font-sans text-[11px] leading-relaxed text-text-dim">{JSON.stringify(report.assumptions, null, 2)}</pre></div><div><h4 className="font-mono text-[9px] text-text-faint uppercase" style={{ fontFamily: "var(--font-mono)" }}>Limitations</h4><pre className="mt-1 whitespace-pre-wrap font-sans text-[11px] leading-relaxed text-text-dim">{JSON.stringify(report.limitations, null, 2)}</pre></div></div><div className="mt-4 border-t border-hairline pt-2 font-mono text-[9px] text-text-faint" style={{ fontFamily: "var(--font-mono)" }}>Agent {report.agent_version} · Model {report.model_version} · Prompt {report.prompt_version} · v{report.version}</div></Panel>;
}
